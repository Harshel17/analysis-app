    ...
    <div style={{ display: "flex", flexDirection: "row" }}>
      ...
      <div className={styles.container}>
            ...
            <div className={styles.parametersGrid}>
              ...
              <p><strong>Description:</strong>
                {isEditing ? (
                  <input
                    ...
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, description: e.target.value }))}
              ...
              <p><strong>Principal:</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.principal || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, principal: parseFloat(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
                  formatCurrency(analysisData.principal)
                )}
              </p>
              <p><strong>Projection Period:</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.projection_period || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, projection_period: parseInt(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
              ...
              <p><strong>Tax Rate:</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.tax_rate || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, tax_rate: parseFloat(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
                  `${analysisData.tax_rate.toFixed(2)}%`
                )}
              </p>
              <p><strong>Additional Deposit:</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.additional_deposit || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, additional_deposit: parseFloat(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
                  formatCurrency(analysisData.additional_deposit)
                )}
              </p>
              <p><strong>Interest Per Week:</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.interest_week || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, interest_week: parseFloat(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
              ...
              <p><strong>Deposit Frequency (weeks):</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.deposit_frequency || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, deposit_frequency: parseInt(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
              ...
              <p><strong>Regular Withdrawal:</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.regular_withdrawal || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, regular_withdrawal: parseFloat(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
                  formatCurrency(analysisData.regular_withdrawal)
                )}
              </p>
              <p><strong>Withdrawal Frequency (weeks):</strong>
                {isEditing ? (
                  <input
                    type="number"
                    value={updatedParameters.withdrawal_frequency || 0}
                    onChange={(e) => setUpdatedParameters(prev => ({ ...prev, withdrawal_frequency: parseInt(e.target.value) }))}
                    className={styles.editInput}
                  />
                ) : (
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {results.map((r, i) => (
                    <tr key={i}>
                      <td>{r.week}</td>
                      <td className={styles.right}>{formatCurrency(r.beginning_balance)}</td>
                      <td className={styles.right}>{formatCurrency(r.additional_deposit)}</td>
                      <td className={styles.right}>{formatCurrency(r.interest)}</td>
                      <td className={styles.right}>{formatCurrency(r.profit)}</td>
                      <td className={styles.right}>{formatCurrency(r.withdrawal)}</td>
                      <td className={styles.right}>{formatCurrency(r.ending_balance)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div style={{ display: "flex", justifyContent: "center", marginTop: "2rem" }}>
          <button onClick={() => router.back()} style={{ padding: "0.5rem 1rem", backgroundColor: "#e5e7eb", borderRadius: "6px", fontWeight: 500, cursor: "pointer" }}>
